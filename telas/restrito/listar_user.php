...
">
	...
	<br>
	<div class="titulo"><h4>Listar Usuários</h4></div>
	<div class="fullcont">
		<div class="medcontadm">
			<ul class="collapsible borda">
            <?php 
                if (isset($_GET['busca'])) {
					$pesquisa = mysqli_real_escape_string($conn, $_GET['busca']);
					$sql_pesquisa = "SELECT * FROM tb_user WHERE nome_user LIKE '%$pesquisa%' OR sobnome_user LIKE '%$pesquisa%' OR tipo_user LIKE '%$pesquisa%' OR end_user OR login_user LIKE '%$pesquisa%' LIKE '%$pesquisa%' ORDER BY tipo_user, nome_user;";
				}else{
				$sql = "SELECT * FROM tb_user ORDER BY tipo_user, nome_user;";
				}
				if (isset($_GET['busca'])) {
            ...
            ?>
				<li>
				    <div class="collapsible-header">
                        <i style="color: #311b92;" class="material-icons" title="<?php 
                        ...
                        <label><?php echo $user['nome_user'] . ' ' . $user['sobnome_user'];?></label>
                    </div>
				        <div class="collapsible-body">
					  <table>
						  <thead>
						  ...
						  <tbody>
							<tr>
								...
								<td><p><?php if($_SESSION['tipo']=="Admin") {echo $user['pass_user'];}else{echo '****';} ?></p></td>
								<td><p><a class="btn excluir borda <?php if($_SESSION['tipo'] != "Admin"){echo 'disabled';}?>" onclick = "return confirm('Deseja realmente excluir?');" href="excluir_produto.php?id=<?php echo $user['id_user'];?>"><i style="color:#fff;"class="material-icons">clear</i></a></p></td>
